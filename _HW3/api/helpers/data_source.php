...
class DataSource{
    ...
    public function JSON($array = true)
    {
        ...
        $this->writePath = $path;
        return json_decode($json, $array); // Return PHP Array ? or return JSON ?
        // json_decode  decode JSON to php object 
    }
    ...
    private function mysqlSetup()
    {
        ...
        try {
            ...
            if (is_array($this->creds)) {
                ...
                $json = file_get_contents($path); //  file_get_contents() return string type 

                $settings = json_decode($json, true);  // json_decode() return JSON string 
            }
